...
class Deepness:
    ...
    def __init__(self, iface: QgisInterface):
        ...
        self._pending_map_processing = []  # queue of MapProcessingParameters to run sequentially
    ...
    def _run_model_inference(self, params: MapProcessingParameters):
        ...
        # If a processing task is already running, enqueue this request to run afterwards
        if self._map_processor and self._map_processor.is_busy():
            self._pending_map_processing.append(params)
            msg = 'Processing queued. It will start when the current task finishes.'
            self.iface.messageBar().pushMessage(PLUGIN_NAME, msg, level=Qgis.Info, duration=5)
            return
    ...
    def _map_processor_finished(self, result: MapProcessingResult):
        ...
        # Collect any dialogs to show after starting the next queued task
        critical_dialog = None
        critical_title = None
        success_msgbox = None
        ...
        if isinstance(result, MapProcessingResultFailed):
            ...
            if result.exception is not None:
                ...
                detailed = f'{msg}\n\n\n' \
                           ...
                           f'Last Traceback: \n' \
                           f'{trace}'
                critical_dialog = detailed
                critical_title = "Unhandled exception"
            ...
            success_msgbox = ResizableMessageBox(self.dockwidget)
            success_msgbox.setWindowTitle("Processing Result")
            success_msgbox.setText(message_to_show)
            success_msgbox.setStyleSheet("QLabel{min-width:800 px; font-size: 24px;} QPushButton{ width:250px; font-size: 18px; }")

        # Free the current processor and start the next queued task BEFORE showing blocking dialogs
        self._map_processor = None
        if self._pending_map_processing:
            next_params = self._pending_map_processing.pop(0)
            self._run_model_inference(next_params)

        # Now show any blocking dialogs (they won't prevent the next task from starting)
        if critical_dialog is not None:
            QMessageBox.critical(self.dockwidget, critical_title or "Error", critical_dialog)
        if success_msgbox is not None:
            success_msgbox.exec()
