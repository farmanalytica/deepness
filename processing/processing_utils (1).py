...
def transform_points_list_xy_to_target_crs(
    ...
    # precisa ter pelo menos 3 vértices para formar geometria válida
    ...
    for point_xy in points:
        # valida se é iterável com pelo menos 2 valores
        if not hasattr(point_xy, "__len__") or len(point_xy) < 2:
            continue
    ...
    # garante novamente mínimo de 3 pontos
    if len(points_crs) < 3:
        return []
    return points_crs
