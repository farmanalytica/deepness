...
class MapProcessorDetection(MapProcessorWithModel):
    ...
    def __init__(self,
        ...
        self.folder = getattr(params, 'output_folder', None)
        self.name = getattr(params, 'layer_name', None)
    ...
    def _run(self) -> MapProcessingResult:
        # If output shapefile already exists, skip processing
        try:
            if self.folder and self.name:
                output_shp = os.path.join(self.folder, f"{self.name}.shp")
                if os.path.exists(output_shp):
                    msg = f"Output exists, skipping processing: {output_shp}"
                    return MapProcessingResultSuccess(message=msg, gui_delegate=None)
        except Exception:
            # if any issue checking the filesystem, continue with processing
            pass
    ...
    def _create_vlayer_for_output_bounding_boxes(self, bounding_boxes: List[Detection]):
        ...
        self.save_output_vlayers(vlayers)

        return add_to_gui

    def save_output_vlayers(self, vlayers):
        print("Saving output vector layers... to ", self.folder)
        os.makedirs(self.folder, exist_ok=True)
        for vlayer in vlayers:
            output_path = os.path.join(self.folder, f"{self.name}.shp")
