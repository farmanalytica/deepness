...
class MapProcessor(QgsTask):
    ...
    def __init__(self,
        ...
        # Validate rlayer units per pixel to avoid division by zero/infinite results
        import math
        if not math.isfinite(self.rlayer_units_per_pixel) or self.rlayer_units_per_pixel <= 0:
            raise OperationFailedException(
                f"Invalid computed rlayer_units_per_pixel: {self.rlayer_units_per_pixel}. "
                "Check `resolution_m_per_px` parameter and raster layer properties.")
