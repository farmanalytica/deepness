...
class DeepnessDockWidget(QtWidgets.QDockWidget, FORM_CLASS):
    ...
    def __init__(self, iface, parent=None):
        ...
        # Guard to prevent re-entrant calls when adding layers triggers signals
        self._folder_path_changing = False
        self.setupUi(self)
        ...
        self.mQgsFileWidget.fileChanged.connect(self.folder_path_changed)
        self.project_instance = QgsProject.instance()
        # Do not react to global layersAdded signal (would cause re-entrancy/loops)
        # self.project_instance.layersAdded.connect(self.folder_path_changed)
        #self.project_instance.layersRemoved.connect(self.folder_path_changed)

    def folder_path_changed(self):
        # Prevent recursive calls: adding layers triggers layersAdded signal
        if getattr(self, '_folder_path_changing', False):
            return

        self._folder_path_changing = True
        folder = self.mQgsFileWidget.filePath()

        try:
            if not folder or not os.path.isdir(folder):
                print(f"Invalid or non-existent folder: {folder}")
                return

            # formatos raster que queremos carregar (apenas nesta pasta, NÃO recursivo)
            extensoes_validas = ('.tif', '.tiff', '.jp2', '.img', '.ecw')
            print(f"Adding raster files from folder (non-recursive): {folder}")

            # evitar adicionar arquivos já presentes no projeto
            existing_sources = set()
            for lyr in QgsProject.instance().mapLayers().values():
                try:
                    existing_sources.add(os.path.abspath(lyr.source()))
                except Exception:
                    continue

            # percorre apenas os itens diretamente na pasta (não recursivo)
            with os.scandir(folder) as it:
                for entry in it:
                    ...
                    name = entry.name
                    if not name.lower().endswith(extensoes_validas):
                        continue

                    path_completo = os.path.abspath(entry.path)

                    # garantir que o arquivo seja exatamente da pasta selecionada
                    if os.path.dirname(path_completo) != os.path.abspath(folder):
                        continue

                    if path_completo in existing_sources:
                        # já adicionado
                        continue

                    layer = QgsRasterLayer(path_completo, name)
                    if layer.isValid():
                        QgsProject.instance().addMapLayer(layer)
                    else:
                        print(f"Falha ao carregar: {path_completo}")
        except Exception as e:
            print(f"Error loading layers from folder {folder}: {e}")
        finally:
            self._folder_path_changing = False
    ...
    def _create_connections(self):
        ...
        self.pushButton_runInference_batch.clicked.connect(self.run_loop)
    ...
    def run_loop(self):
        ...
        layer_count = self.mMapLayerComboBox_inputLayer.count()
        for i in range(layer_count):
            self.mMapLayerComboBox_inputLayer.setCurrentIndex(i)
            #time.sleep(2)
            print(f"Processing layer {i + 1} of {layer_count}: {self._get_input_layer().name()}")

            try:
                self._run_inference()
            except Exception as e:
                print(f"Error processing layer {self._get_input_layer().name()}: {e}")
                continue  # Proceed to the next layer despite the error
    ...
    def get_detection_parameters(self, map_processing_parameters: MapProcessingParameters) -> DetectionParameters:
        ...
        params.output_folder = self.mQgsFileWidget_2.filePath()
        params.layer_name = self.mMapLayerComboBox_inputLayer.currentLayer().source().split(os.sep)[-1].split('.')[0]
